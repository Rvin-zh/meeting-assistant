...
class TestClassifyGeminiHttpError:
    ...
    def test_rate_limit(self):
        ...
        assert status == 429
        assert "سهمیه" in detail


class TestHttpExceptionFromGeminiError:
    def test_dunning_returns_persian_billing_message(self):
        exc = ModelHTTPError(
            status_code=403,
            model_name="gemini-2.5-flash",
            body=DUNNING_BODY,
        )
        http_exc = http_exception_from_gemini_error(exc, context="پردازش")
    ...
    def test_non_gemini_error_uses_generic_message(self):
        ...
        assert "خطا در RAG" in http_exc.detail
