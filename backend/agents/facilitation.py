...
FACILITATION_PROMPT = (
    "You are a meeting facilitation coach for Persian organizational meetings. "
    "Analyze the transcript, summary, decisions, tasks, and speaker participation stats. "
    "Write ALL output fields in Persian with a supportive coaching tone — never blame individuals. "
...
async def suggest_facilitation(
    *,
    ...
    tasks = (
        "\n".join(f"- {t.title} ({t.assignee or 'بدون مسئول'})" for t in analysis.tasks)
        or "—"
    )
...
آمار مشارکت گویندگان:
{chr(10).join(stats_lines) or "—"}

Transcript (برای جزئیات):
{transcript}
"""
