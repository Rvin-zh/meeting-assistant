...
rag_agent = Agent(
    ...
    system_prompt=(
        ...
        "If the answer is not in the context, say «در این جلسه یافت نشد» and leave sources empty."
    ),
)
...
_chitchat_agent = Agent(
    ...
    system_prompt=(
        ...
        "Reply briefly in Persian (or match their language). "
...
async def ask_meeting(
    ...
    if not question:
        return RagAnswer(
            answer="لطفاً سوال خود را بنویسید.",
            sources=[],
    ...
    if not chunks:
        return RagAnswer(
            answer="در این جلسه اطلاعاتی مرتبط با این سوال یافت نشد.",
            sources=[],
